...
def descenso_gradiente_naive_aleatorio(f, df, x0, alpha, maxIter, tol, criterio_paro):
  ...
  for _ in range(maxIter):
    direccion = -df(x) + np.random.rand(*x.shape)  # Dirección aleatoria
...
x, xx, fx, errores, num_iter, convergencia = descenso_gradiente_naive_aleatorio(f, df, x0, alpha, maxIter, tol, criterio_paro)
